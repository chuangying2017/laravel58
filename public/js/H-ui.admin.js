    ...
    layer.confirm('确认要停用吗？',function(index){
        $.ajax({
            ...
            success: function(data){
                if (data.status == 1)
                {
                    $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="member_start(this,\''+id+'\',\''+url+'\')" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1;</i></a>');
                    $(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius">待用</span>');
                    $(obj).remove();
...
/*用户-启用*/
function member_start(obj,id,url){
    layer.confirm('确认要启用吗？',function(index){
        $.ajax({
            ...
            success: function(data){
              if (data.status == 1)
              {
                  $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="member_stop(this,\''+id+'\',\''+url+'\')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>');
                  $(obj).parents("tr").find(".td-status").html('<span class="label label-success radius">活跃</span>');
                  $(obj).remove();
